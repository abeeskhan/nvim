...
local createCustomLspCommands = function ()
    vim.cmd("command! LspDefenition lua vim.lsp.buf.definition()");
...
local LspKeymaps = {
    LspGotoDefenition = "gd",
    LspRename = "gr",
    LspReferences = "gR",
    LspTypeDefenition = "gY",
    LspHover = "K",
    LspOrganizeImports = "goi",
    LspDiagnosticsGoPrevious = "[a",
    LspDiagnosticsGoNext = "]a",
    LspCodeAction = "ga",
    LspDiagnosticsShowLine = "<Leader>a",
    LspSignatureHelp = "<C-x><C-x>",
}

local createLspKeymappings = function(BufferMap, BufferNumber)
    local keymapOptions = {silent=true}
    BufferMap(BufferNumber, VIM_MODES.Normal, LspKeymaps.LspGotoDefenition, "<cmd>LspDefenition<CR>", keymapOptions);
    ...
    BufferMap(BufferNumber, VIM_MODES.Normal, LspKeymaps.LspDiagnosticsGoPrevious, "<cmd>LspDiagnosticsGoPrevious<CR>", keymapOptions);
    BufferMap(BufferNumber, VIM_MODES.Normal, LspKeymaps.LspDiagnosticsGoNext, "<cmd>LspDiagnosticsGoNext<CR>", keymapOptions);
    BufferMap(BufferNumber, VIM_MODES.Normal, LspKeymaps.LspCodeAction, "<cmd>LspCodeAction<CR>", keymapOptions);
    BufferMap(BufferNumber, VIM_MODES.Normal, LspKeymaps.LspDiagnosticsShowLine, "<cmd>LspDiagnosticsShowLine<CR>", keymapOptions);
...
local onAttach = function (Client, BufferNumber)
    ...
    createLspKeymappings(vim.api.nvim_buf_set_keymap, BufferNumber);
